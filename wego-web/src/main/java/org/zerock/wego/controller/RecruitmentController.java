...
public class RecruitmentController {
	...
	@PostMapping("/modify")
	public String modify(RecruitmentDTO dto, RedirectAttributes rttrs) throws ControllerException { // 모집글 수정 요청처리
	...
	public String recruitmentUpload(String sanName, // 산이름
		...
		try {
			// 산이름으로 산ID 조회
			Integer sanId = this.service.sanNameSelect(sanName);
			dto.setSanInfoId(sanId);
		} catch (Exception e) {
			throw new ControllerException(e);
		} // try-catch

		Integer userId = 130; // 테스트용 유저 ID
